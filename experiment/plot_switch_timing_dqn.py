...
Direct DQN counterpart to plot_switch_timing.py (PPO-only) -- same
structure, same guarantees, so the two plots are visually and
methodologically comparable side by side in the report even though
they're generated by separate scripts. Use plot_switch_timing_multi.py
instead if you want both algorithms overlaid in one figure; this one is
for a standalone DQN-section plot, matching how the PPO-only plot is
used in the PPO section.

Model path is built from train_dqn.py's own MODELS_DIR constant
(import train_dqn.py directly) rather than a separately hardcoded
folder name -- same reasoning as plot_switch_timing.py's docstring:
hardcoded paths here have already gone stale more than once. Reward
config and min_green are likewise sourced from traindqnmod's constants,
...
DQN requires the FlattenMultiDiscreteAction wrapper (Discrete(4) action
space) that PPO doesn't -- see wrappers.py. The wrapper only touches
actions; info["switched"] still comes straight from the base env, so
switch-event logging here is apples-to-apples with the PPO plot.
"""
...
MODEL_PATH      = os.path.join(traindqnmod.MODELS_DIR, "dqn_sumo_2junction")
NORMALIZER_PATH = os.path.join(traindqnmod.MODELS_DIR, "vec_normalize_sumo_dqn.pkl")
MAX_STEPS       = 3600
N_EPISODES      = 5
TL_IDS          = ["J1", "J2"]
BIN_SIZE        = 100
...
def make_env(seed):
    def _init():
        env = SumoTrafficEnv2J(
            ...
            max_steps=MAX_STEPS,
            seed=seed,
            switch_penalty=traindqnmod.SWITCH_PENALTY,
            wasted_vote_penalty=traindqnmod.WASTED_VOTE_PENALTY,
            imbalance_bonus_weight=traindqnmod.IMBALANCE_BONUS_WEIGHT,
            wrong_direction_penalty=traindqnmod.WRONG_DIRECTION_PENALTY,
            min_green=traindqnmod.MIN_GREEN,
        )
...
def main():
    if not os.path.exists(MODEL_PATH + ".zip"):
        sys.exit(f"FATAL: no DQN model found at {MODEL_PATH}.zip -- train it with train_dqn.py first")
    ...
    # actually passed in, since both come from traindqnmod.
    sample_env = base_env.envs[0]
    while hasattr(sample_env, "env"):   # unwrap VecEnv/Monitor/action wrappers if present
    ...
    run_label = f"DQN_sp{hp['switch_penalty']}_wd{hp['wrong_direction_penalty']}_mingreen{hp['min_green']}"
    base_env.close()
    ...
    for ax, tl in zip(axes, TL_IDS):
        ax.hist(all_switch_steps[tl], bins=bins, color="darkorange", edgecolor="white")
